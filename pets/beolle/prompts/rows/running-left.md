Create a single horizontal sprite strip for the Codex app digital pet `beolle` in the state `running-left`.
...
Animation action: leftward 데굴데굴 rolling loop. The caterpillar rolls its long soft body to the left across the 8 frames, matching the rightward rolling personality while clearly facing/traveling left.


State-specific requirements:
- Show leftward rolling locomotion through body rotation, segment tilt, and head/body orientation changes only.
