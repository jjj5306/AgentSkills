Create a single horizontal sprite strip for the Codex app digital pet `beolle` in the state `jumping`.
...
Animation action: 물구나무서기 loop. The caterpillar tips forward, balances briefly upside down on its head/top area, then returns to its normal pose. Keep it plush, cute, and readable rather than athletic.


State-specific requirements:
- Show the handstand through body orientation only: normal pose, tipping forward, upside-down balance, tipping back, normal pose.
- Keep the whole body within each frame slot during the upside-down pose. The head and top antennae may point downward during inversion because the entire pet is rotated, but do not add any new lower antennae or underside appendages.
