Create a single horizontal sprite strip for the Codex app digital pet `beolle` in the state `running-right`.
...
Animation action: rightward 데굴데굴 rolling loop. The caterpillar rolls its long soft body to the right across the 8 frames, like a plush caterpillar happily tumbling in place from slot to slot. Show body rotation and segment tilt through pose changes only.


State-specific requirements:
- Show rightward rolling locomotion through body rotation, segment tilt, and head/body orientation changes only.
- Keep the two antennae attached to the top of the head in every frame; do not create lower antennae or underside limbs while rolling.
