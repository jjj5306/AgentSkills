Create a single horizontal sprite strip for the Codex app digital pet `beolle` in the state `waving`.
...
Output exactly 4 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Turn the attached plush caterpillar into a Codex digital pet named 벌레. Preserve the approved corrected base: pale pink face, black oval eyes with tiny white highlights, red dot nose, brown head band, exactly two short pink antennae only on top of the head, no antennae/arms/appendages on the lower head or underside, gently long horizontal caterpillar body, and soft rainbow body bands in pink, green, sky blue, and pink. Personality: gentle Korean-first developer companion, careful with planning, docs, and reviews; cozy, curious, steady, lightly playful..

Style contract: Codex digital pet sprite style: pixel-art-adjacent low-resolution mascot sprite, compact chibi proportions, chunky whole-body silhouette, thick dark 1-2 px outline, visible stepped/pixel edges, limited palette, flat cel shading with at most one small highlight and one shadow step, simple readable face, tiny limbs, and no detail that disappears at 192x208. Avoid polished illustration, painterly rendering, anime key art, 3D render, vector app-icon polish, glossy lighting, soft gradients, realistic fur or material texture, anti-aliased high-detail edges, and complex tiny accessories. Additional user style notes: Codex built-in digital pet style: compact chibi sprite, pixel-art-adjacent, thick dark 1-2 px outline, flat cel shading, limited palette, clean silhouette, no text, no UI, no scenery, no shadows..
...
Animation action: 혀 낼름 loop. The caterpillar briefly sticks out a tiny red tongue and pulls it back in, playful and cute, while keeping the body mostly still.


State-specific requirements:
- Show the tongue flick through mouth/tongue pose only: tongue hidden, tiny red tongue peeking out, tongue fully out, tongue hidden again.
- The tongue must be small, attached to the mouth, opaque, hard-edged, and inside the face silhouette area. Do not draw a detached tongue, saliva, droplets, symbols, motion marks, sparkles, or floating effects.
- Keep the two antennae attached to the top of the head in every frame; do not create lower antennae or underside limbs.
