Create a single horizontal sprite strip for the Codex app digital pet `pengdori` in the state `waiting`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: An original compact chibi water-penguin digital pet, inspired by a cute pocket-monster penguin archetype but not copying any existing Pokemon character. Round blue head and body, white face patch, tiny yellow beak and feet, dark 1-2 px outline, small blue gear clasp on the scarf with no letters, numbers, terminal marks, or symbols, calm diligent expression for CT analysis and PowerShell work. No logos, no text, no exact Pokemon markings..
